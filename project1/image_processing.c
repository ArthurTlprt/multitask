...
{
    fwrite(img->raster[img->height/nmax*(n-1)], sizeof(unsigned char),
    img->height/nmax* img->width, fp);
}
...
void convolve(struct image *img, kernel_t *kernels[2], struct image *out, int n,
  int nmax)
{
    for (int row = img->height/nmax*(n-1); row < img->height/nmax*n; ++row)
    {
...
// perform convolution on n processes
int multi_process(int n,int nmax,struct image* img,struct image* out,
   kernel_t *kernels[2],FILE *fp)
{
    if (n == 0) {
        return 0;
    }
    ...
    if (pid==0) {
        ...
        n = n-1;
        multi_process(n,nmax,img,out,kernels,fp);
        exit(0);
    }
    else {
       wait(NULL);
...
int main(int argc, char *argv[])
{
    if (argc !=3 && argc !=4 )
    {
        fprintf(stderr, "Usage: %s filename1 filename2 [optionnal] n_processe(s)\n", argv[0]);
        return EXIT_FAILURE;
    }
